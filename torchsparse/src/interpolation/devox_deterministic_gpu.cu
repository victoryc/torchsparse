...
// c)
__global__ void
deterministic_devoxelize_kernel(int N, int c, const int *__restrict__ indices,
...
__global__ void deterministic_devoxelize_grad_kernel(
    ...
    const float *__restrict__ weight, const float *__restrict__ top_grad,
    int *__restrict__ bottom_grad) {
...
void deterministic_devoxelize_wrapper(int N, int c, const int *indices,
                                      const float *weight, const float *feat,
                                      float *out) {
  deterministic_devoxelize_kernel<<<N, c>>>(N, c, indices, weight, feat, out);
}

void deterministic_devoxelize_grad_wrapper(int N, int n, int c,
                                           const int *indices,
                                           const float *weight,
                                           const float *top_grad,
                                           int *bottom_grad) {
  deterministic_devoxelize_grad_kernel<<<N, c>>>(N, n, c, indices, weight,
                                                 top_grad, bottom_grad);
}
