...
// c)
__global__ void devoxelize_kernel(int N, int c, const int *__restrict__ indices,
...
__global__ void devoxelize_grad_kernel(int N, int n, int c,
                                       const int *__restrict__ indices,
                                       const float *__restrict__ weight,
                                       const float *__restrict__ top_grad,
                                       float *__restrict__ bottom_grad) {
...
void devoxelize_wrapper(int N, int c, const int *indices, const float *weight,
                        const float *feat, float *out) {
  devoxelize_kernel<<<N, c>>>(N, c, indices, weight, feat, out);
}

void devoxelize_grad_wrapper(int N, int n, int c, const int *indices,
                             const float *weight, const float *top_grad,
                             float *bottom_grad) {
  devoxelize_grad_kernel<<<N, c>>>(N, n, c, indices, weight, top_grad,
                                   bottom_grad);
}
